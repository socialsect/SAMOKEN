...
const MAX_PUTTS = 3;
const API_URL = "https://19364d24b176.ngrok-free.app/analyze-ball";
...
useEffect(() => {
    ...
    const startCamera = async () => {
        ...
        try {
            // Detect if this is likely a back camera
            const isBackCamera = device.label.toLowerCase().includes('back') || 
                               device.label.toLowerCase().includes('rear') ||
                               device.label.toLowerCase().includes('environment');
            setIsUsingBackCamera(isBackCamera);
            ...
            if (videoRef.current) {
                ...
                videoRef.current.onloadedmetadata = () => {
                    ...
                    console.log('Using back camera:', isBackCamera);
                    setIsLoading(false);
...
useEffect(() => {
    ...
    const processFrame = async () => {
        ...
        try {
            ...
            // FIXED: Don't flip the canvas when sending to API
            // The API should receive the raw camera feed
                ...
                try {
                    ...
                    if (detections?.length > 0) {
                        ...
                        if (validDetections.length > 0) {
                            ...
                            // FIXED: Only flip for front camera (when video is mirrored)
                            // For back camera, use coordinates as-is
                            let finalX = adjustedX;
                            if (!isUsingBackCamera) {
                                // Front camera - flip horizontally to match mirrored display
                                finalX = video.videoWidth - adjustedX;
                            }
                            
                            const scaledX = displayInfo.offsetX + (finalX * displayInfo.scale);
                            const scaledY = displayInfo.offsetY + (adjustedY * displayInfo.scale);
                            
                            // FIXED: Update bounding box with same logic
                            if (bestDet.box && bestDet.box.length === 4) {
                                ...
                                let finalX1 = adjX1, finalX2 = adjX2;
                                if (!isUsingBackCamera) {
                                    // Front camera - flip bounding box
                                    finalX1 = video.videoWidth - adjX2;
                                    finalX2 = video.videoWidth - adjX1;
                                }
                                
                                lastDetectionBox.current = [
                                    displayInfo.offsetX + (finalX1 * displayInfo.scale),
                                    displayInfo.offsetY + (adjY1 * displayInfo.scale),
                                    displayInfo.offsetX + (finalX2 * displayInfo.scale),
...
}, [isRecording, shouldProcessFrame, isValidBallDetection, getVideoDisplayDimensions, isMobile, isUsingBackCamera]);
...
useEffect(() => {
    ...
    const drawFrame = () => {
        ...
        if (isMobile) {
            ...
            ctx.textAlign = "left";
            ctx.fillText(
                isUsingBackCamera ? "Back Camera" : "Front Camera", 
                10, 
                canvasDimensions.height - 10
            );
        }
    };
...
return (
    <div style={styles.container}>
        ...
        <video 
            ...
            style={{
                ...styles.video,
                transform: isUsingBackCamera ? "none" : "scaleX(-1)"
            }} 
            muted 
            playsInline 
            autoPlay 
        ...
        <div style={styles.mobileControls}>
            ...
            <div style={styles.mobileStatusBar}>
                ...
                <div style={styles.statusItem}>
                    Camera: {isUsingBackCamera ? 'Back' : 'Front'}
                </div>
            </div>
        </div>
